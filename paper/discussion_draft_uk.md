...
### 3. Target-user WISDM adaptation

Після початкового `balanced_600` sanity run стало зрозуміло, що змішаний subset
по всіх користувачах не є достатньо сильним доказом continual learning. Він
перевіряє device-side inference path, але не відповідає на питання
персоналізації:

```text
train users != target user
target user має помітний baseline error
adaptation використовує labeled target-user samples
post-adaptation evaluation виконується на held-out target-user windows
```

Тому було додано staged target-user gate. PC-side screening кількох users
показав, що `user=19` є найкориснішим сценарієм: загальна baseline accuracy
приблизно `71%`, а `Downstairs` практично не розпізнавався. Після цього
fold-specific artifact для `user=19` було перенесено на ESP32 в isolated binary.

Device-side result:

```text
held-out pre-adaptation accuracy:  73.03%
held-out post-adaptation accuracy: 80.26%
...
Це змінює силу статті. Тепер робота має не лише real-device MPU6050 pilot, а й
clean WISDM target-user CL proof-of-concept на ESP32. Це все ще не повний LOSO
benchmark, але це прямий evidence для основної тези: RAM-only supervised replay
adaptation може покращити held-out target-user behavior на самій платі.

Важливо не перебільшувати цей результат. Він показаний на одному target user,
і FIFO/reservoir дали однаковий результат у цьому protocol. Коректний claim:

```text
The target-user run demonstrates feasible device-side adaptation and weak-class
recovery on one held-out WISDM user.
```

Некоректний claim:

```text
The method is statistically superior to prior HAR CL methods across WISDM.
```

### 4. Why MicroFlow-32 became the primary embedded path
...
### 5. Interpretation of FIFO vs reservoir

У поточному real-device pilot обидва replay modes змістили prediction
distribution на upstairs-like segment після supervised labels. `FIFO` досяг
`88.57%` accepted-rate для upstairs-like segment, а `reservoir-per-class` -
`93.94%`. У target-user WISDM run для `user=19` обидві policy дали однаковий
held-out gain: `73.03% -> 80.26%`.
...
Водночас поточні experiments занадто малі для твердження, що reservoir
статистично кращий за FIFO. Reservoir показав трохи сильніший результат у
MPU6050 pilot, але не відрізнився від FIFO у `user=19` WISDM target-user run.
...
альтернативну replay policy з тим самим RAM budget, а не як доведено superior.
...
### 6. What the real-device pilot demonstrates
...
### 7. Domain shift and why adaptation is needed
...
### 8. Label acquisition and UART limitations
...
### 9. Why persistence is future work
...
7. **No statistical FIFO/reservoir conclusion.** Поточні pilot results
   ...
   перевагу reservoir.
8. **Single target-user WISDM CL result.** `user=19` є сильним proof-of-concept,
   але не замінює повний LOSO CL benchmark по всіх users.
...
- розширити target-user CL evaluation з `user=19` до повного LOSO-style sweep;
...
> and target-user weak-class recovery on WISDM user 19.
...
> shift у pilot з реальним MPU6050 і покращення held-out target-user behavior
> для WISDM `user=19`.
